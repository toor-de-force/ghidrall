...
import copy
class WL:
    #PERFORM WL algorithm node count number of iterations
    def __init__(self,g):
    ...
    def to_canonical(self):
        ...
        for i in number_of_iterations:
            for i in N:
                ...
                new_labels[i] = self.get_new_labels()
            nx.set_node_attributes(self.graph,new_labels,"wl_colour")
            self.update_multisets()
            self.update_histogram()
        pass
    def get_new_labels(self):
        old_colour = nx.get_node_attributes(self.graph,"wl_colour")
        old_multiset = nx.get_node_attributes(self.graph,"neighbour_multiset")
