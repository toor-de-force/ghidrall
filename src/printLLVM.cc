...
bool PrintLLVM::hasCharTerminator(uint1 *buffer,int4 size,int4 charsize)
...
bool PrintLLVM::printCharacterConstant(ostream &s,const Address &addr,int4 charsize) const
...
/// \param vn is the Varnode holding the value
/// \param op is the PcodeOp using the value
void PrintLLVM::pushCharConstant(uintb val,const TypeChar *ct,const Varnode *vn,const PcodeOp *op)
...
bool PrintLLVM::isCharacterConstant(const uint1 *buf,int4 size,int4 charsize) const
...
void PrintLLVM::emitGlobalVarDeclsRecursive(Scope *scope)
